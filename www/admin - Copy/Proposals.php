<?php
 ...
 $sql="SELECT p.ID,p.DepID,d.DepartmentName,p.IdeaNames,p.Description,p.Proposal,p.TeacherAssign,p.Students,p.StudentNumber,p.MarksObtained,p.IsCompleted,p.Ratings,p.Status,DATE_FORMAT(p.DateAdded, '%M %d %Y') as Added,DATE_FORMAT(p.DateModified, '%M %d %Y') as Modified FROM ideas p LEFT JOIN departments d ON p.DepID = d.ID WHERE p.ID <> 0";
	 ...
	 //if value is not null  and deletion form is submitted
 {
	...
	// echo $keyvalues;
		$sql = "DELETE FROM ideas WHERE ID IN (".$keyvalues.")";//WHERE ID is in this array means all selected ID's will be deleted together
		// echo $sql;
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              ...
              <h3 class="box-title">Proposals Management</h3>
			   <!--Add and Delete buttons-->
			  <button type="button" class="btn btn-block btn-primary" style="width: auto;float: right;" onclick='location.href="AddProposal.php"'>Add</button>
              ...
              <table id="example2" class="table table-bordered table-hover">
				...
				<!--First <td> for deletion purpose-->
				 ...
				 <td><?php echo $rows['DepartmentName']?></td>
                  <td><?php echo $rows ['IdeaNames']?></td>
				  <td><?php echo $rows ['Description']?></td>
				  <td><?php echo $rows ['Proposal']?></td>
				  <td><?php echo $rows ['Students']?></td>
				  <td><?php echo $rows ['StudentNumber']?></td>
				  <td><?php echo $rows ['IsCompleted']?></td>
				  <td><?php echo $rows ['Ratings']?></td>
				  <td><?php echo $rows ['Status']?></td>
				  <td><?php echo $rows['Added']?> </td>
				  <td><?php echo $rows['Modified']?> </td>
				  <td><button type="button" class="btn btn-block btn-primary" style="width: auto;float: right;" onclick='location.href="EditProposal.php?EID=<?php echo $rows['ID']?>"'>Edit</button></td>
