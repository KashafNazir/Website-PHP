<?php
	 ...
	 //if value is not null and deletion form is submitted
 {
	...
	// echo $keyvalues;
		$sql = "DELETE FROM roles WHERE ID IN (".$keyvalues.")";//WHERE ID is in this array means all selected ID's will be deleted together
		// echo $sql;
        ...
        <div class="col-xs-12">
          <div class="box">
			...
			<!---The data will be displayed on this page-->
			  ...
			  <button type="button" class="btn btn-block btn-primary" style="width: auto;float: right;" onclick='location.href="AddRole.php"'>Add</button>
			...
			//The mysql_fetch_array() is used to retrieve a row of data as an array from a MySQL result handle.
				...
				<!--First <td> for deletion purpose-->
				 <td><input type="checkbox" class="CIds" name="Ids[]" value="<?php echo $rows['ID']?>"/></td>
				  ...
				  <td><button type="button" class="btn btn-block btn-primary" style="width: auto;float: right;" onclick='location.href="EditRole.php?EID=<?php echo $rows['ID']?>"'>Edit</button></td>
